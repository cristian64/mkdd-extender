...
#if RESET_COURSE_PAGE_ON_TITLE_SCREEN >= 0 || RESET_COURSE_PAGE_ON_LAN_INITIALIZATION >= 0
...
static void draw_course_and_author_name()
{
    do
    {
...

        {
            ...
            draw_text(x, y, course_name, 0xFFD40800, opacity);
        }
        ...
        if (s_course_name_and_author_opacity > 0x00FF)  // Overflow
            break;

        // Normal exit; function will be called again.
        return;
    } while (false);

    // Hide overlay if reached.
...
#if RESET_COURSE_PAGE_ON_LAN_INITIALIZATION >= 0
void lanplayinfo_init_ex()
{
    LANPlayInfo__init();
    set_course_page(RESET_COURSE_PAGE_ON_LAN_INITIALIZATION);
}
#endif
