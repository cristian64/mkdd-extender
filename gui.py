...
def set_dark_theme(app: QtWidgets.QApplication):
    ...
    role_colors.append((QtGui.QPalette.PlaceholderText, QtGui.QColor(90, 90, 90)))
    ...
    def _on_course_ripper_action_triggered(self):
        ...
        description_label.setText(
            markdown_to_html(
                ...
                'Double Dash!! ISO file. Its purpose is to let users freely build their own '
                'personal ISO with the custom courses that they enjoy.'
                '\n\n'
                'Although the tool strives to reconstruct courses accurately, the process is not '
                'flawless and will require manual intervention. For example, course names and '
                'author names cannot be inferred automatically: the user must ultimately edit the '
                '`trackinfo.ini` file in each course to provide this information.'
                '\n\n'
                'Some custom courses may depend on cheat codes or code patches that the tool will '
                'not be able to rip. One way to determine whether a given ripped course has such '
                'dependencies is to use MKDD Extender (or MKDD Patcher) to place it in a new ISO '
                'and the play the course: if the course can be played fully without crashing, it '
                '_probably_ does not require any cheat code or code patch. For courses that do '
                'require cheat codes or code patches, there are two possible solutions:'
                '\n\n'
                '- Try to reach out to the original author and ask for details.\n'
                '- For more experienced users, examine the BOL file (the one that can be edited '
                '  in MKDD Track Editor) or the BCO file (the one that can be converted with the '
                '  `mkdd-collision` tool) to see if they contain properties that do not appear in '
                '  the original game. If so, edit those properties out and try to play the course '
                '  again.',
            ))
        layout.addWidget(description_label)
        layout.addSpacing(dialog.fontMetrics().height())
